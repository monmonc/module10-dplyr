...
flights <- mutate(flights, time.amount = arr_delay - dep_delay)
...
jfk.sea <- flights %>% 
           ...
           summarize(min = min(air_time, na.rm = TRUE), max = max(air_time, na.rm = TRUE), average = mean(air_time, na.rm = TRUE))
